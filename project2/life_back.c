...
t_simulate(void* arg)
{
    ...
    for(gen = 0; gen < num_gens; gen++)
    {
        ...
        //printf("thread [%d:%d] synchronized to gen %d\n",input->start,input->end,gen);         
        
        //printf("thread [%d:%d] starting gen %d\n",input->start,input->end,gen);   
        int i;
        ...
        pthread_mutex_unlock(&thread_complete_mut);*/
        //printf("thread [%d:%d] finished gen %d\n",input->start,input->end,gen);       
    }
}
...
main(int argc, char ** argv)
{
    ...
    //printf("size: %d threads: %d gens: %d input: %s output: %s\n",
    //    grid_size,num_threads,num_gens,input,output);
    ...
    for(i=0;i<num_threads-1;i++)
    {
        ...
        //printf("thread %d works on range [%d:%d]\n",i,arg_list[i].start,arg_list[i].end);
    }
    int main_start = start;
    int main_end = start + step + extra -1;
    //printf("main thread works on range [%d:%d]\n",main_start,main_end);
